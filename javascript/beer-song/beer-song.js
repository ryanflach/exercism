...
class BeerSong {
  verse(verseNum) {
    const pluralCurrentWall =
      `${verseNum}${bottlesOnWall}, ${verseNum}${bottles}`;
    const pluralUpdatedWall =
      `${takeDown}${verseNum - 1}${bottlesOnWall}.\n`;
    const newlySingleUpdatedWall =
      `${takeDown}${verseNum - 1}${bottleOnWall}.\n`;
    const singularCurrentWall =
      `${verseNum}${bottleOnWall}, ${verseNum}${bottle}`;
    ...
    if (verseNum > 2) {
      ...
      return (
        `No more${bottlesOnWall}, no more${bottles}Go to the store and buy some more, 99${bottlesOnWall}.\n`
      );
    }
  }

  sing(startVerse, endVerse = 0) {
    let verses = '';
